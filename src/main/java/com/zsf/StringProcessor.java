...
import com.zsf.tool.ExpressionComparator;
...
public class StringProcessor {
     ...
     * @param partitions
     */
    private ExpressionGroup predictOutput(String newInput, List<ExampleCluster> partitions) {
...
//                if (v.getTargetString().equals(((NonTerminalExpression) expression).interpret(v.getInputString())))
    ...
    public List<ExpressionGroup> combinateSubPrograms(List<ResultMap> resultMaps, int k) {
        ...
        RunTimeMeasurer.endTiming("combinateSubPrograms");
        return ansList;
    }
     ...
     * @param examplePairs
     */
    public List<ExampleCluster> formatClustering(List<ExpressionGroup> expressionGroups, List<ExamplePair> examplePairs) {
        // init
        RunTimeMeasurer.startTiming();
        List<ExampleCluster> partitions = new ArrayList<ExampleCluster>();
        for (int i = 0; i < examplePairs.size(); i++) {
            partitions.add(new ExampleCluster(examplePairs.get(i), expressionGroups.get(i)));
        }

        boolean needMerge = true;
        while (needMerge) {
            ExampleCluster partition1 = null;
            ExampleCluster partition2 = null;
            ...
            if (needMerge) {
                ...
                partitions.add(new ExampleCluster(pairs, p12TheSameExpressions));
            }
        }
     ...
     * @return
     */
    private static ExpressionGroup findSameExps(ExampleCluster partition1, ExampleCluster partition2) {
...
//                            }
     ...
     * @return
     */
    public static int lookupPartitionIndex(String string, List<ExampleCluster> partitions) {
        int index = -1;
        double maxScore = -1;
        for (int i = 0; i < partitions.size(); i++) {
     ...
     * @return
     */
    public static ExpressionGroup getTopNExpressions(ExampleCluster partition, String testString, int n) {
